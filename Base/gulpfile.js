...
const imgHTML = require('gulp-avif-webp');
...
const imgOptimized = require("gulp-sharp-responsive");
...
const configPath = {
  app : {
    ...
    imgWebpAvif : [app + 'img/**/*.+(jpeg|jpg|png)', '!' + app + 'img/svg/*.svg'],
    svg : app + 'img/svg/*.svg',
...
function webServer () {
  ...
  watch((configPath.watch.imgWebpAvif), series(imgWeb)).on('change', browserSync.reload)
...
function imgWeb () {
	return src(configPath.app.imgWebpAvif)
...
exports.build = series(toBuild, clear, parallel(htmlTask, imgTask, imgWeb, svgTask, fontsOldTask, fontsTask, assetsTask, scriptsTask), fontStyleTask, stylesTask);
